...
internal class TestDescendant : SizzleTest {
	
	Void testDescendantSingle() {
		...
		elems = doc.select("html p .tom")
		verifyEq(elems.size, 1)
		verifyElem(elems[0], "span", "tom")

		elems = doc.select("html p .tom .dick")
